...
import com.Minecraft.DeStilleGast.TogglePVP.Main;
...
public class CombatLog implements org.bukkit.event.Listener {
	...
	private ArrayList<PlayerTagExtentie> tagList = new ArrayList<PlayerTagExtentie>();
	...
	public PlayerTagExtentie getPlayerTag(Player player){
		for(PlayerTagExtentie p : tagList){
	...
	public ArrayList<PlayerTagExtentie> getTagList() {
		return tagList;
	}

	public void onJoin(Player p){
		if(getPlayerTag(p) == null){
			tagList.add(new PlayerTagExtentie(p));
		}
	}
	...
	public void onAttack(Player attacker, Player target){
		...
		final PlayerTagExtentie pAttacker = getPlayerTag(attacker);
		final PlayerTagExtentie pTarget = getPlayerTag(target);
						...
						if(pAttacker.getAttackerList().size() == 0){
							...
							if(pAttacker.getAttackerList().size() > 1){
								for(int i = 0; i < pAttacker.getAttackerList().size() - 1; i++){
									...
									if((pAttacker.getAttackerList().size() - 1) > i){attackers += " ,";}
								}	
	...
	public void onPlayerDeath(Player p){
		PlayerTagExtentie victum = getPlayerTag(p);
